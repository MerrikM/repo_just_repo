...
const PROBABILITY_SCORE_CHANGED = 0.1 // для теста выбрано (0.1) 10%, по умолчанию было: 0.0001
...
func main() {
	...
	testOffsets := []int{0, 10, 123, 999, 50000, 100000}
	for _, off := range testOffsets {
		score := getScore(*stamps, off)
		fmt.Printf("%v: %v -- %v\n", off, score.Home, score.Away)
	}
}
